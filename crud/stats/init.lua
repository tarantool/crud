...
--- Observe an `atomic_batch` sub-operation explicitly.
--
--  Unlike `stats.observe`, records pure execution time of a single
--  sub-operation measured on storage (see `crud.atomic_batch`),
--  not the whole request latency on router.
--
-- @function observe_atomic_batch_sub_op
--
-- @number latency
--  Sub-operation latency in seconds.
--
-- @string space_name
--  Space name.
--
-- @string op
--  Sub-operation label (see `crud.stats.op`).
--
-- @string status
--  `'ok'` or `'error'`.
--
-- @treturn boolean Returns `true`.
--
function stats.observe_atomic_batch_sub_op(latency, space_name, op, status)
    dev_checks('number', 'string', 'string', 'string')
    ...
    internal:get_registry().observe_atomic_batch_sub_op(latency, space_name, op, status)
