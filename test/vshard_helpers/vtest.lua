...
local cluster = require('test.vshard_helpers.cluster')
...
local SOCKET_DIR = fio.abspath(os.getenv('VARDIR') or '/tmp/t')
local function instance_uri(alias, instance_id)
    ...
    for replicaset_name, replicaset_templ in pairs(templ.sharding) do
        ...
        for replica_name, replica_templ in pairs(replicaset_templ.replicas) do
            ...
            if replica_templ.instance_uuid ~= nil then
                replica_uuid = replica_templ.instance_uuid
            end
            ...
            if port_count == nil then
                replica.uri = creds..instance_uri(replica_name)
            else
                local listen = table.new(port_count, 0)
                for j = 1, port_count do
                    listen[j] = creds..instance_uri(replica_name..j)
                end
...
local function cluster_new(g, cfg)
    ...
    cluster_bootstrap(g, cfg)
    cluster_wait_vclock_all(g)
end
...
local function cluster_cfg(g, cfg)
    ...
    local _, err = cluster_exec_each(g, function(cfg)
        return require('vshard.storage').cfg(cfg, box.info.uuid)
    end, {cfg})
...
local function cluster_rebalancer_disable(g)
    local _, err =  cluster_exec_each(g, function()
        require('vshard.storage').rebalancer_disable()
    end)
...
local function cluster_rebalancer_enable(g)
    local _, err =  cluster_exec_each(g, function()
        require('vshard.storage').rebalancer_enable()
    end)
...
local function storage_wait_follow_f(id)
    ...
    while true do
        ...
        do return end
    ::retry::
        if fiber.clock() > deadline or status == 'stopped' then
            t.fail(yaml.encode({
                err = last_err,
                dst_id = id,
...
local function router_disconnect(router)
    router:exec(function()
        local replicasets = require('vshard.router').static.replicasets
...
local sourcedir = fio.dirname(debug.sourcedir())
